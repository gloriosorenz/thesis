...
$(document).ready(function () {
    ...
    function addRow()
    {
        var addRow='<tr><td><select class="form-control" name="rice_farmers_id[]" id="rice_farmers_id"><option value="0" selected="true" disabled="True"></option>@foreach ($rice_farmers as $key=>$p)<option value="{{$key}}">{{$p}}</option>@endforeach</select></td>' +
                    '<td><input type="text" class="form-control" name="planned_hectares[]" value="" /></td>' +
                    '<td><input type="text" class="form-control" name="planned_num_farmers[]" value="" /></td>' +
                    '<td><input type="text" class="form-control" name="planned_qty[]" value="" /></td>' +
                    '<td><input type="text" class="form-control" name="actual_hectares[]" value="" /></td>' +
                    '<td><input type="text" class="form-control" name="actual_num_farmers[]" value="" /></td>' +
                    '<td><input type="text" class="form-control" name="actual_qty[]" value="" /></td>' +
                    '<td><input type="button" class="btn btn-danger remove" value="x"></td></tr>';
        $('tbody').append(addRow);
    }

    $('.resultbody2').delegate('.remove', 'click', function () {
        var l=$('tbody tr').length;
        if(l==1)
        {
            alert('You can not remove last one');
